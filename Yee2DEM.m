...
for i = 1:nSteps
    ...
    for k = 1:Reg.n
        ...
        if bc{k}.NumS > 0
            for loop = 1:2
                Einc = bc{k}.s(loop).fct(yC{k},i,t,bc{k}.s(loop).paras);
                Eincp = bc{k}.s(loop).fct(yC{k},i,t-ds/c_c/2,bc{k}.s(loop).paras);

                Hy{k}(bc{k}.s(loop).xpos,:) = Hy{k}(bc{k}.s(loop).xpos,:) + dt/ds/c_mu_0*Einc;
                Ez{k}(bc{k}.s(loop).xpos-1,:) = Ez{k}(bc{k}.s(loop).xpos-1,:) - dt/ds/c_eps_0/c_eta_0*Eincp;
            end 
        end
